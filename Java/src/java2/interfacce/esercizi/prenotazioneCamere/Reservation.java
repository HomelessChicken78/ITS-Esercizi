...
public class Reservation implements Comparable<Reservation> {
	...
	Reservation(String nomeCliente, int inizioPrenotazione, int finePrenotazione) {
		this.nomeCliente = nomeCliente;
		
		if (inizioPrenotazione < 1 || inizioPrenotazione > 365)
			throw new RuntimeException("L'inizio deve essere compreso tra 1 e 365 inclusi.");
		if (finePrenotazione < 1 || finePrenotazione > 365)
			throw new RuntimeException("La fine deve essere compresa tra 1 e 365 inclusi.");
		if (inizioPrenotazione > finePrenotazione)
			throw new RuntimeException("L'inizio deve venire prima della fine!");
